...
function generateRandomString(length) {
  ...
  return randomString;}
async function mixNumbersInString(randomString) {try {
    ...
    let Index = 0;
    let charCount = 0;
    for (let i = 0; i < randomString.length; i++) {mixedString += randomString[i];charCount++;if (charCount === interval && Index < Numbers.length) {mixedString += Numbers[Index];
        Index++;
        charCount = 0;}}
    while (Index < Numbers.length) {
      mixedString += Numbers[Index];
      Index++;}
    return mixedString;} catch (error) {
...
function sendMessage() {
  ...
  .then(function(response) {
    if (response.ok) {
      lastMessageTime = currentTime;
      toastNotify("Successfully sent your message!", "Thanks!");} else {
...
function toastNotify(alt, alm, alttype, altclr) {
  ...
  setTimeout(() => {
    ...
    toast.classList.remove("ntact");}, 5000);
  setTimeout(() => {
    ntprog.classList.remove("ntact");}, 5300)}
